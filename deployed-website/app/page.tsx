...
export default function Page() {
  ...
  useEffect(() => {
    async function syncPantryBuckets() {
      ...
      const upsertBucket = async <T,>(bucketName: string, payload: T): Promise<T> => {
        await fetch(`${PANTRY_BASE_URL}/${bucketName}`, {
          method: "POST",
      ...
      try {
        const [aboutBucket, flowBucket, imagesBucket, footerContactBucket] = await Promise.all([
          upsertBucket<AboutBucket>("about", aboutPayload),
          upsertBucket<FlowBucket>("flow", flowPayload),
          upsertBucket<ImagesBucket>("images", imagesPayload),
          upsertBucket<FooterContactBucket>("footer-contact", footerContactPayload),
        ])
