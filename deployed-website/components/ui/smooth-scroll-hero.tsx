        ...
        >
          <div className="text-center text-white max-w-4xl mx-auto px-6">
            ...
            <h2 className="text-4xl md:text-6xl lg:text-7xl font-black tracking-wider mb-6 leading-none">
              {content.join.ctaTitle}
              <br />
              <span className="bg-gradient-to-r from-white via-gray-200 to-white bg-clip-text text-transparent">
                {content.join.ctaSubtitle}
              </span>
            </h2>

            {/* Supporting Text */}
            <p className="text-lg md:text-xl lg:text-2xl text-gray-200 mb-8 leading-relaxed font-medium">
              {content.join.ctaDescription}
            </p>

            {/* Stats Grid */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
              {content.join.stats.map((stat, idx) => (
                <div key={idx} className="text-center">
                  <div className="flex justify-center mb-2">
                    <div className="w-10 h-10 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center">
                      {stat.icon === "users" && <Users className="w-5 h-5 text-white" />}
                      {stat.icon === "map-pin" && <MapPin className="w-5 h-5 text-white" />}
                      {stat.icon === "calendar" && <Calendar className="w-5 h-5 text-white" />}
                      {stat.icon === "trophy" && <Trophy className="w-5 h-5 text-white" />}
                    </div>
                  </div>
            ...
            {/* CTA Button */}
            <a href={content.join.ctaLink} target="_blank" rel="noopener noreferrer">
              <LiquidButton
                size="xxl"
                className="font-bold text-xl tracking-wide px-12 py-4 bg-gray-900 hover:bg-gray-800 text-white border-2 border-gray-900 hover:scale-105 transition-all duration-300 cursor-pointer"
              >
                {content.join.ctaButtonText}
              </LiquidButton>
            </a>
            ...
            <div className="mt-12 pt-6 border-t border-white/20">
              ...
              <div className="flex flex-wrap justify-center items-center gap-4 text-gray-300">
                {content.join.trustIndicators.map((indicator, idx) => (
